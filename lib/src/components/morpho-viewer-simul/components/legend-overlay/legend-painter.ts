...
export class LegendPainter {
	...
	paint(canvas: HTMLCanvasElement | null, targets: LegendTarget[]) {
		...
		for (const target of targets) {
			targetIndex++;
			const sectionId = resolveSectionId(target.section);
...
/**
 * Names can be `dend[32]` and in this case, we want to return `32`.
 */
function resolveSectionId(section: string) {
	const start = section.indexOf("[");
	if (start === -1) return section;

	const end = section.indexOf("]");
	return section.slice(start + 1, end !== -1 ? end : undefined);
}
